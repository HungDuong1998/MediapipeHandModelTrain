...
from model import PointHistoryClassifierFullhand1
...
def main():
    ...
    point_history_classifier_fullhand = PointHistoryClassifierFullhand1()
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if point_history_len == (history_length * 6):
                    ...
                    print(point_history_fullhand_classifier_labels[finger_gesture_id_fullhand])
                ...
                debug_image = draw_info_text(
                    ...
                    point_history_fullhand_classifier_labels[most_common_fg_id_fullhand[0][0]],
                )
        else:
...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 3 and (0 <= number <= 9):
        csv_path = 'model/point_history_classifier/point_history_fullhand.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *point_history_list])
    return
...
def draw_info_text(image, brect, handedness, hand_sign_text,
                   finger_gesture_text, hand_gesture_text):
    ...
    if hand_gesture_text != "":
        cv.putText(image, "Hand Gesture:" + hand_gesture_text, (10, 90),
                   cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 4, cv.LINE_AA)
        cv.putText(image, "Hand Gesture:" + hand_gesture_text, (10, 90),
...
def draw_point_history(image, point_history):
    for index, point in enumerate(point_history):
        if len(point) == 4:
            ...
            cv.circle(image, (point[2], point[3]), 1 + int(index / 2),(152, 251, 152), 2)
        if len(point)==6 :            
            cv.circle(image, (point[0], point[1]), 1 + int(index / 2),(152, 251, 152), 2)
            cv.circle(image, (point[2], point[3]), 1 + int(index / 2),(152, 251, 152), 2)
            cv.circle(image, (point[4], point[5]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[6], point[7]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[8], point[9]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[10], point[11]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[12], point[13]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[14], point[15]), 1 + int(index / 2),(152, 251, 152), 2)
    #         cv.circle(image, (point[16], point[17]), 1 + int(index / 2),(152, 251, 152), 2)
    return image
...
def draw_info(image, fps, mode, number):
    ...
    mode_string = ['Logging Key Point', 'Logging Point History', 'Logging Point All Hand History']
    if 1 <= mode <= 3:
